...
import qualified Data.Primitive.PrimArray as PArray
...
data Document = Document
  { rawInput :: {-# unpack #-} !Text
  , utf8Encoded :: {-# unpack #-} !ByteString
  , indexable :: {-# unpack #-} !(PrimArray Char) -- for O(1) indexing pos -> Char
  , firstNonAdjacent :: {-# unpack #-} !(PrimArray Int)
  ...
  , tDropToUtf16Drop :: {-# unpack #-} !(PrimArray Int)
...
it pays off to cache UTF-8 the encoded ByteString. That's the utf8Encoded
field in Document.
...
fromText rawInput = Document{..}
  ...
  indexable = primArrayFromListN rawInputLength unpacked
  firstNonAdjacent = primArrayFromListN rawInputLength
    $ snd
    $ foldr gen (rawInputLength, [])
    $ zip [0..] unpacked
  ...
  tDropToBSDrop = primArrayFromListN (rawInputLength + 1) tDropToBSDropList
  tDropToUtf16Drop = primArrayFromListN (rawInputLength + 1)
    $ scanl' (\acc a -> acc + utf16CharWidth a) 0 unpacked
  bsDropToTDrop = primArrayFromListN (BS.length utf8Encoded + 1)
    $ reverse
    $ snd
    $ foldl' fun (-1, [])
    $ zip [0..] tDropToBSDropList
...
isAdjacent (Document{..}) a b =
  b >= a && (indexPrimArray firstNonAdjacent a >= b)

isAdjacentSeparator :: Char -> Bool
isAdjacentSeparator c = elem c [' ', '\t']

(!) :: Document -> Int -> Char
(!) (Document {..}) ix = indexPrimArray indexable ix

length :: Document -> Int
length (Document {..}) = sizeofPrimArray indexable
...
byteStringFromPos Document {..} position
  ...
  rangeToText r = UText.takeWord16 (end16Pos - start16Pos) $
    ...
    where
    start16Pos = indexPrimArray tDropToUtf16Drop startPos
    end16Pos = indexPrimArray tDropToUtf16Drop endPos
  ...
  translateRange !bsStart !bsLen = startPos `seq` endPos `seq` res
    ...
    startPos = indexPrimArray bsDropToTDrop realBsStart
    endPos = indexPrimArray bsDropToTDrop realBsEnd

primArrayFromText :: Text -> PrimArray Char
primArrayFromText txt = runST go
  where
    go :: forall s. ST s (PrimArray Char)
    go = do
      buf <- newPrimArray (Text.length txt)
      Text.foldl' -- smh no foldM
        (\ixM c -> do
            ix <- ixM
            writePrimArray buf ix c
            pure (ix + 1)
        )
        (pure 0)
        txt
